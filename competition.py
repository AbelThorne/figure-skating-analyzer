...
class Competition(object):
    ...
    def __str__(self) -> str:
        ...
        nb_categories = (
            len(self._master_table) if self._master_table is not None else "N/A"
        )
        nb_score_cards = (
            len(
                list(
                    filter(
                        lambda f: f.is_file() and f.suffix == ".pdf",
                        list(self._scores_dir.iterdir()),
                    )
                )
            )
    ...
    def from_dir(
        self, path: Path, crawl_to_complete: bool = False, set_as_save_dir: bool = True
    ) -> "Competition":
        info_missing = False
        ...
        if path.joinpath("info.json").exists():
            with open(path.joinpath("info.json"), "r") as f:
                ...
                self._start = (
                    date_parse(info["start"]).date() if "start" in info else None
                )
                ...
                self._scores_dir = (
                    path.joinpath(info["scores_dir"]) if "scores_dir" in info else None
                )
                self._parsed_dir = (
                    path.joinpath(info["parsed_dir"]) if "parsed_dir" in info else None
                )
                if (
                    self._url is None
    ...
    def download_scores(
        self, dest_path: Path = None, overwrite: bool = False, relative=False
    ) -> dict:
        ...
        if relative and self._save_dir is None:
            print(
                "Cannot set scores path relative to save directory: save directory is None"
            )
            return None
        ...
        for cat in self._master_table.values():
            for seg in cat["segments"]:
                ...
                if "scores_link" in seg:
                    ...
                    if (
                        overwrite
                        or not self._scores_dir.joinpath(
    ...
    def parse_scores(self, dest_path: Path, overwrite=False, relative=True):
        ...
        if relative and self._save_dir is None:
            print(
                "Cannot set parsed path relative to save directory: save directory is None"
            )
            return None
        ...
        while index < len(rows):
            ...
            if cells[0].text != "":
                ...
                category["entries_link"] = urljoin(
                    self.url, cells[2].find("a").attrs.get("href")
                )
                category["results_link"] = urljoin(
                    self.url, cells[3].find("a").attrs.get("href")
                )
                ...
                if (
                    ...
                    try:
                        segment["scores_link"] = urljoin(
                            self.url, cells[4].find("a").attrs.get("href")
                        )
    ...
    def _infer_type(self) -> None:
        ...
        if match is not None:
            type = dict(
                filter(lambda elem: elem[1] is not None, match.groupdict().items())
            )
            if len(type) == 1:
