...
def tidify_competition(competition):
    ...
    for i, p in enumerate(competition["performances"]):
        ...
        p["metadata"]["total_base_value"] = p["elements"]["total"]["base_value"]
        p["metadata"]["unfactored_components"] = sum([c["scores_of_panel"] for c in p["components"].values()])
        p["metadata"]["avg_unfactored_components"] = np.average([c["scores_of_panel"] for c in p["components"].values()])
...
def competitions_to_csvs(list_of_competitions, dest="."):
    ...
    performances_df = pd.DataFrame(tidy.performances)[[
            ...
            'total_base_value',
            'unfactored_components',
            'avg_unfactored_components',
            'total_deductions',
        ]].assign(
...
if __name__ == "__main__":
    competitions = [ json.load(open(path))
        for path in Path("output/json").glob("**/*.json") ]

    competitions_to_csvs(competitions, "output/tidy")
